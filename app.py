...
def package_endpoint():
    ...
    # Generate variants and store files temporarily
    ...
    with ZipFile(zip_path, 'a', ZIP_DEFLATED) as zipf:  # Append README
        zipf.writestr("README.txt", readme_content)

    # Serve the zip and clean up (optional)
    try:
        response = send_file(zip_path, as_attachment=True, download_name=zip_name)
        # Optional: Delete individual files after serving (uncomment to enable)
        # for file_path in temp_files:
        #     if os.path.exists(file_path):
        #         os.remove(file_path)
        return response
    finally:
        # Always clean up the zip file after serving
