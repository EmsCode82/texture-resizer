...
def package_endpoint():
    ...
    # Generate variants (mimics /profile/gameasset defaults for now)
        ...
        if pbr and 'pbr' in results:
            for size in results['pbr']:
                for map_type in ['normal', 'roughness']:
                    ...
                    path = os.path.join(OUTPUT_DIR, url)
                    zipf.write(path, os.path.join("PBR", url))
