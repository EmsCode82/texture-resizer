...
def package_endpoint():
            ...
            for size in results[fmt]:
                ...
                if base_url:
                    ...
                    if os.path.exists(file_path):  # Ensure file exists
                        ...
                        logger.debug(f"Added base to temp_files: {file_path} (format: {fmt}, size: {size})")
    
    # Extract nested PBR from png results (where it's generated)
    if pbr:
        if 'png' in results:
            for size in results['png']:
                size_str = str(size)
                if 'pbr' in results['png'][size_str]:
                    ...
                    if size_str in pbr_data:
                        size_pbr = pbr_data[size_str]
                        for map_type in ['normal', 'roughness']:
                            if map_type in size_pbr:
                                file_info = size_pbr[map_type]
    ...
    # Cleanup after zipping (files will be deleted even if download succeeds)
    for file_path in temp_files:
        if os.path.exists(file_path):
            os.remove(file_path)
